import { useState } from 'react'
import { GoogleOAuthProvider, useGoogleLogin } from '@react-oauth/google'
import { API_BASE_URL } from '../api/config'
import { isNative, signInWithGoogleNative } from '../native'

const GOOGLE_CLIENT_ID = (import.meta.env.VITE_GOOGLE_CLIENT_ID as string | undefined) || '406462367912-qsnckspm7sepdi011ubrvo80q49l1ovf.apps.googleusercontent.com'

interface GoogleAuthButtonProps {
  onAuth: (user: any, token: string) => void
  onError: (message: string) => void
  /** Return false to abort the sign-in attempt (e.g. terms not yet accepted). */
  beforeStart?: () => boolean
  label?: string
}

async function exchangeWithBackend(body: Record<string, string>, onAuth: GoogleAuthButtonProps['onAuth']) {
  const response = await fetch(`${API_BASE_URL}/oauth/google`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(body),
  })
  const data = await response.json()
  if (!response.ok) throw new Error(data?.message || 'Google sign-in failed')
  localStorage.setItem('wanderworkToken', data.token)
  localStorage.setItem('wanderworkUser', JSON.stringify(data.user))
  onAuth(data.user, data.token)
}

function GoogleIcon() {
  return (
    <svg width="18" height="18" viewBox="0 0 48 48">
      <path fill="#EA4335" d="M24 9.5c3.54 0 6.71 1.22 9.21 3.6l6.85-6.85C35.9 2.38 30.47 0 24 0 14.62 0 6.51 5.38 2.56 13.22l7.98 6.19C12.43 13.72 17.74 9.5 24 9.5z"/>
      <path fill="#4285F4" d="M46.98 24.55c0-1.57-.15-3.09-.38-4.55H24v9.02h12.94c-.58 2.96-2.26 5.48-4.78 7.18l7.73 6c4.51-4.18 7.09-10.36 7.09-17.65z"/>
      <path fill="#FBBC05" d="M10.53 28.59c-.48-1.45-.76-2.99-.76-4.59s.27-3.14.76-4.59l-7.98-6.19C.92 16.46 0 20.12 0 24c0 3.88.92 7.54 2.56 10.78l7.97-6.19z"/>
      <path fill="#34A853" d="M24 48c6.48 0 11.93-2.13 15.89-5.81l-7.73-6c-2.18 1.48-4.97 2.35-8.16 2.35-6.26 0-11.57-4.22-13.47-9.91l-7.98 6.19C6.51 42.62 14.62 48 24 48z"/>
      <path fill="none" d="M0 0h48v48H0z"/>
    </svg>
  )
}

function GoogleButtonShell({ loading, onClick, label }: { loading: boolean; onClick: () => void; label?: string }) {
  return (
    <button
      type="button"
      onClick={onClick}
      disabled={loading}
      className="w-full flex items-center justify-center gap-3 py-3 rounded-xl border border-gray-300 bg-white hover:bg-gray-50 transition font-semibold text-sm text-gray-700 disabled:opacity-60 disabled:cursor-not-allowed"
    >
      {loading ? (
        <span className="w-4 h-4 rounded-full border-2 border-gray-400 border-t-transparent animate-spin" />
      ) : (
        <GoogleIcon />
      )}
      {label || 'Continue with Google'}
    </button>
  )
}

function NativeGoogleButton({ onAuth, onError, beforeStart, label }: GoogleAuthButtonProps) {
  const [loading, setLoading] = useState(false)

  const handleClick = async () => {
    if (beforeStart && !beforeStart()) return
    setLoading(true)
    try {
      const idToken = await signInWithGoogleNative()
      await exchangeWithBackend({ credential: idToken }, onAuth)
    } catch (err: any) {
      onError(err?.message || 'Google sign-in failed')
    } finally {
      setLoading(false)
    }
  }

  return <GoogleButtonShell loading={loading} onClick={handleClick} label={label} />
}

function WebGoogleButton({ onAuth, onError, beforeStart, label }: GoogleAuthButtonProps) {
  const [loading, setLoading] = useState(false)

  const googleLogin = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      setLoading(true)
      try {
        await exchangeWithBackend({ accessToken: tokenResponse.access_token }, onAuth)
      } catch (err: any) {
        onError(err?.message || 'Google sign-in failed')
      } finally {
        setLoading(false)
      }
    },
    onError: () => onError('Google sign-in was cancelled'),
  })

  return (
    <GoogleButtonShell
      loading={loading}
      label={label}
      onClick={() => {
        if (beforeStart && !beforeStart()) return
        googleLogin()
      }}
    />
  )
}

export default function GoogleAuthButton(props: GoogleAuthButtonProps) {
  if (isNative) return <NativeGoogleButton {...props} />
  return (
    <GoogleOAuthProvider clientId={GOOGLE_CLIENT_ID}>
      <WebGoogleButton {...props} />
    </GoogleOAuthProvider>
  )
}
